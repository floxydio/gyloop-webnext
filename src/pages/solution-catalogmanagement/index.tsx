...
export async function getServerSideProps(context) {
  ...
  const fetchHeader = await axios.get(
    `${url}/v1/product-header/get?lang_code=${context.locale}&page_code=catalog_management`
  );
  const fetchSolutionFunction = await axios.get(
    `${url}/v1/solution/function?page_code=catalog_management&lang_code=${context.locale}`
  );
  const fetchSolutionProduct = await axios.get(
    `${url}/v1/solution/product?lang_code=${context.locale}&page_code=catalog_management`
  );

  const fetchMediaHighlight = await axios.get(
    `${url}/v1/main/media?lang_code=${context.locale}&page_code=catalog_management`
  );
