...
export async function getServerSideProps(context) {
  ...
  const fetchHeader = await axios.get(
    `${url}/v1/product-header/get?lang_code=${context.locale}&page_code=business_networking`
  );

  const fetchSolutionFunction = await axios.get(
    `${url}/v1/solution/function?lang_code=${context.locale}&page_code=business_networking`
  );

  const fetchSolutionProduct = await axios.get(
    `${url}/v1/solution/product?lang_code=${context.locale}&page_code=business_networking`
  );

  const fetchMediaHighlight = await axios.get(
    `${url}/v1/main/media?lang_code=${context.locale}&page_code=business_networking`
  );
